...
ScalableColorDescriptor::~ScalableColorDescriptor()
{
   // std::cout << "Run ~ScalableColorDescriptor()" <<std::endl;
...
float ScalableColorDescriptor::distance(ScalableColorDescriptor* scd)
{
	...
	{
		unsigned long scd_coeff = scd->GetCoefficient(i);
		unsigned long scd_sign = scd->GetCoeffSign(i);
		// dist += std::abs( m_Sign[i]*m_ScalableHistogram[i] - m_Sign[i] * m_ScalableHistogram[i]);
		dist += (m_Sign[i] * m_ScalableHistogram[i] > scd_sign * scd_coeff) ? \
			(m_Sign[i] * m_ScalableHistogram[i] - scd_sign * scd_coeff) :\
			(scd_sign * scd_coeff - m_Sign[i] * m_ScalableHistogram[i]);
	}
    return dist;
}
